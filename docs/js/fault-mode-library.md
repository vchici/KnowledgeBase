...
5. 命中"原生层风险清单"中的条目时，若栈中出现引擎或原生模块帧，即使 debug 包也无法反解其源码行（引擎/SDK/三方 native 库为预编译 release 形态），需索取引擎符号，或借助 HybridStack 中可读的 JS 帧定界。
...
| `JSError` | `RangeError`       | `Stack overflow!`                                                                                                                          | 栈溢出错误                                                           | 混合:JS为主,常由原生回调重入JS触发 |
...
| `JSError` | `ReferenceError`   | `cannot find record '<name>' in basefileName '<filename>',from napi load module`                                                           | 模块查找失败                                                          | C++/NAPI:napi 加载原生模块失败 |
...
| `JSError` | `Error`            | `The underlying ArrayBuffer is null or detached.`                                                                                          | ArrayBuffer被分离或释放                                               | C++/NAPI:底层原生内存被释放/分离 |
| `JSError` | `Error`            | `The ArkTS Map's constructor cannot be directly invoked.`                                                                                  | 不可直接调用map的构造函数                                                  |             |
| `JSError` | `OutOfMemoryError` | `OutOfMemory when trying to allocate <size> bytes function name: <name>, <heap-type> oom, total size <size> bytes, used size <size> bytes` | ArkTS虚拟机堆内存不足，产生OOM异常                                           | 引擎原生:VM堆为原生内存,分配失败点在引擎内部,debug 包看不到引擎帧源码行。用户提供 rawheap 或 heapsnapshot 时，调用 `jsleak-analysis` Skill 继续分析。 |
| `JSError` | `TerminationError` | `Terminate execution!`                                                                                                                     | 虚拟机被强制终止执行。可通过故障日志中的Stacktrace或HybridStack字段查看调用来源              | 引擎原生/系统:进程被强制终止,终止动作可能来自 native/系统侧 |

## 原生层风险清单

上表"分析要点 / 修复方向"列对可能涉及原生层的条目做了标注。命中这些条目时，**即使 debug 包也可能无法查看源码行**，原因：引擎/SDK/三方 native 库是预编译 release 形态，与你的 debug/release 配置无关。

| 标注 | 含义 | debug 包能否看到源码行 |
| --- | --- | --- |
| (无标注) | 纯 JS 层异常，栈顶为 JS 帧 | ✅ 未压缩 + debug 可直接源码级分析 |
| `混合` | JS 帧为主，但可能混入原生帧 | ⚠️ 混入的原生帧看其来源（引擎/工程 C++） |
| `C++/NAPI` | 涉及原生模块/SDK 加载或原生内存生命周期 | ⚠️ SDK/三方库 debug 也看不到源码行；工程自身 C++ 代码 debug 可看 |
| `引擎原生` | 崩溃点可能在 JS 引擎（ark_js_vm）内部 | ⚠️ 引擎为预编译 release，debug 也反解不出引擎帧源码行，需引擎符号或厂商支持 |

标注仅表示"可能存在原生层参与"，不能只凭错误类型定界，仍需结合 Stacktrace/HybridStack 确认责任代码。
