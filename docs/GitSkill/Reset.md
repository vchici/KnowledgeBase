> 撤销最近多次 commit 有三种方式，不同点在保留还是丢弃 commit 的修改：

# 1. `git reset --soft`（保留修改，放回暂存区）

```bash
# 撤销最近 3 次 commit，但代码修改保留在暂存区，可以重新编辑后再提交。
...
```bash
# 撤销最近 3 次 commit，代码修改放回工作区（未暂存状态）。这是默认行为，`--mixed` 可以省略。
...
`reset` 本意是将Head指针指回之前的某个提交，区别是将被撤销的代码修改，分别吐到哪个区域。
...
修改已有文件（撤销后显示为 `modified`），因为 Git 的历史记录里认得这个文件，所以它知道这个文件被“修改”了，状态会显示为 **`modified`**

全新添加的文件（撤销后显示为 `untracked`），在“过去”的那个时间点，**这个文件在 Git 的世界里根本不存在**。
...
但这会影响其他协作者，谨慎使用。更推荐用 `git revert`，它会创建新的 commit 来"反转"指定 commit 的修改，不改历史：
...
这种方式更安全，已经 push 过的仓库推荐用 [Revert](./Revert.md) 而不是 `reset`。